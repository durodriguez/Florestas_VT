...
describe('buildDataset — notes never reach the browser', () => {
  it('sends no notes field at all', () => {
    // Not stripped on the way out — never sent. A surveyor's remarks are
    // internal, and not sending them is a stronger guarantee than filtering.
    const r = build({ observationRows: [observation({ notes: 'Leaning badly over the path' })] });
    expect(r.plants.fields).not.toContain('notes');
    expect(r.plants.observationFields).not.toContain('notes');
  });

  it('carries no note text into anything the browser loads', () => {
    const r = build({
      observationRows: [
        observation({ surveyed_on: '2026-09-01', notes: 'SPECIES CHANGED: was Picea abies' }),
        observation({ surveyed_on: '2028-10-14', notes: 'Leaning badly over the path' }),
      ],
    });
    expect(r.errors).toEqual([]);
    for (const payload of [JSON.stringify(r.plants), JSON.stringify(r.dataset)]) {
      expect(payload).not.toContain('SPECIES CHANGED');
      expect(payload).not.toContain('Leaning badly');
    }
  });
});
