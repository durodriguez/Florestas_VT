...
Open items first, finished ones after them. **Status lives in the heading, in
square brackets, and nowhere else** — a second copy is a second thing to go
stale, which is how this file got untidy in the first place.
...
| 14 | [165 trees the two inventories disagree about](#14-165-trees-the-two-inventories-disagree-about-open-fieldwork) | **open** |
...
| 4 | [A "fun fact" column](#4-a-fun-fact-column-done-12-september-2026) | done |
| 5 | [Species autocomplete in the survey app](#5-species-autocomplete-in-the-survey-app-done-11-september-2026) | done |
| 6 | [Finalise the detail panel](#6-finalise-the-detail-panel-done-18-september-2026) | done |
| 8 | [Matching surveyed trees to untagged mapped trees](#8-matching-surveyed-trees-to-untagged-mapped-trees-done-21-september-2026) | done |
| 11 | [Updating the data over time](#11-updating-the-data-over-time-done-11-september-2026) | done |
| 12 | [Burlington's street trees, inside the campus boundary](#12-burlingtons-street-trees-inside-the-campus-boundary-done-22-september-2026) | done |
| 16 | [The full 2014 download](#16-the-full-2014-download-answered-22-september-2026) | answered |

## Settled: the 2023–24 layer is the spine [25 September 2026]

Your call, and it closes a question that had been sitting under several of
these. The ArcGIS layer stays as the map's backbone. It gets corrected in
place through the survey app — positions adjusted, photos added, phantoms
marked — rather than replaced by a fresh inventory.

That changes how two items read. **14** and **15** were partly evidence for
*which dataset to trust*; they are now plain fieldwork worklists and block
nothing. The 2014 file keeps the job it already has: the thing
`check:inventories` and `check:coverage` compare against, and the thing the
survey app autofills from.

It also raises the stakes on **18**, because the survey app is now the
instrument the whole correction strategy runs on.
...
## 9. Renumbering tags by campus [open: the label, not the numbering]
...
## 10. Photos without overloading the repo [part done, 21 September 2026]
...
## 13. Re-tagging: what happens when a tree gets a new number [open: ask grounds]
...
## 14. 165 trees the two inventories disagree about [open: fieldwork]
...
## 15. 1,552 of the 2014 tags are not on the map [open: fieldwork]
...
**Cost.** The accounting is written and tested, and `data/coverage-gaps.csv`
ranks the gaps whenever somebody wants them. Walking them was considered and
**dropped on 25 September 2026** — the answer would be interesting and changes
nothing anyone is going to do.
...
## 18. Two versions of the survey app — advanced and basic [open]
...
- **Whether the basic tier sees the "Is the tree there?" card at all** — hiding
  ...
  evidence [#15](#15-1552-of-the-2014-tags-are-not-on-the-map-open-fieldwork) wants. A crew
...
- `Height` — **Small / Medium / Large**, not a measurement. DBH already says
  size, and says it better. i-Tree ([#7](#7-ecosystem-services--the-i-tree-question-blocked-no-diameters))
...
Things that need a person rather than a commit.
...
- **Tag 762** needs an in-person check: 13 m position disagreement and a species
  ...
  import records this on the tree's 2023 observation, so it is in the data
