...
Burlington's street trees inside the campus boundary. **Not part of the UVM
collection**, and kept in their own file so that nothing which belongs to the
collection — `npm run import`, `npm run labels`, the accession sequence — can
reach them.

`city_id`, `taxon_id`, `lat`, `lng`, `collection_id`, `dbh_in`, `condition`,
`planted_year`, `address`, `recorded_on`, `source_id`.

Generated by `npm run import:city` from the city's open data and rewritten
whole on each refresh, so nothing in it is hand-edited. Validated like every
other file: an unknown `taxon_id`, a bad coordinate, a condition outside the
vocabulary or a `collection_id` with no matching row all fail the build.

It reaches the browser as `dataset.cityTrees`, its own array — never folded
into `plants`, because every count and facet in the app takes "plants" to mean
the university's collection. Full reasoning in
